...
- Limitation: There is one limitation of the dataset -- the total sample size is not large, and some states with smaller sample sizes may show extremely high internet access percentages due to only a few responses from those areas. To make the results more reliable, larger-scale studies are needed in the future.

### 4. Interactive map
Using the **Folium** package, this study visualizes the distribution of internet access among older adults across various U.S. states, integrating data from the AARP database with TIGER/Line shapefiles. This interactive map allows the audience to explore geographic disparities in internet accessibility among different demographic groups within the states. The "Basemap" displays internet access rates by state. Additionally, the "US Demographic Internet Access Map" features 32 different layers, each representing categories of older adults based on education, marital status, race/ethnicity, and whether they live in a metropolitan area. Users can select the group they are interested in to examine the geographic disparities in internet accessibility more closely.
